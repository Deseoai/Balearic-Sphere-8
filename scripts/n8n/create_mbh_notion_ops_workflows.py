...
""".strip()
    ...
    return {
        "name": "MBH - 01 Event Hub (Full Notion Router)",
        "nodes": [
            {
                "id": "webhook-in",
                "name": "Webhook In",
...
def application_poller_workflow():
    js_code = f"""
...
""".strip()
    ...
    return {
        "name": "MBH - 12 HITL Applications Poller",
        "nodes": [
            {
                "id": "schedule",
        ...
        "connections": {
            "Schedule Trigger": {"main": [[{"node": "Poll + Sync", "type": "main", "index": 0}]]}
        },
        "settings": {},
    }


def upgrade_poller_workflow():
    js_code = f"""
const notionToken = (typeof process !== 'undefined' && process.env) ? process.env.NOTION_TOKEN : '';
if (!notionToken) {{
  return [{{ json: {{ ok: false, reason: 'NOTION_TOKEN missing in n8n env' }} }}];
...
""".strip()
    ...
    return {
        "name": "MBH - 13 HITL Upgrades Poller",
        "nodes": [
            {
                "id": "schedule",
                "name": "Schedule Trigger",
...
""".strip()
    ...
    return {
        "name": "MBH - 90 Global Error Logger",
        "nodes": [
            {
                "id": "error-trigger",
                "name": "Error Trigger",
...
payloads = [
    full_router_workflow(),
    application_poller_workflow(),
    upgrade_poller_workflow(),
    error_logger_workflow(),
]
...
out_path = Path("scripts/n8n/workflows.notion-ops.created.json")
