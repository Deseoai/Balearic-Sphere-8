...
def notion_http_node(name: str, node_id: str, position: list, body_expr: str):
    return {
        "id": node_id,
        "name": name,
...
return [{{json: {{ event, eventId, target, notionBody: {{ parent: {{ database_id: DB[target] }}, properties }} }} }}];
""".strip()

    return {
        "name": "MBH - 40 Event Hub (V4 Final)",
        "nodes": [
            {
                "id": "webhook-in",
                "name": "Webhook In",
                ...
                "typeVersion": 2,
                "position": [220, 300],
                "webhookId": "mbh-v4-events-full",
                "parameters": {"httpMethod": "POST", "path": "mbh/v4/events/full", "responseMode": "responseNode", "options": {}},
            },
            {
            ...
            notion_http_node("Create Notion Page", "create-notion-page", [860, 300], "={{$json.notionBody}}"),
            {
                "id": "respond-ack",
                "name": "Respond ACK",
                ...
                "typeVersion": 1.3,
                "position": [1160, 300],
                "parameters": {"respondWith": "json", "responseBody": "={{ { status: 'accepted', event: $json.event, eventId: $json.eventId, mappedDatabase: $json.target } }}", "options": {}},
            },
        ],
        "connections": {
...
def hitl_v4(name: str, path: str, webhook_id: str, decision_type: str):
    js_code = f"""
const payload = ($json && $json.body && typeof $json.body === 'object') ? $json.body : ($json || {{}});
...
""".strip()
    ...
    return {
        ...
        "nodes": [
            ...
            notion_http_node("Create Webhook Event", "create-webhook-event", [800, 200], "={{$json.webhookEventPage}}"),
            notion_http_node("Create Audit Log", "create-audit-log", [1080, 200], "={{$json.auditLogPage}}"),
            {
                "id": "respond-ack",
                "name": "Respond ACK",
                "type": "n8n-nodes-base.respondToWebhook",
                "typeVersion": 1.3,
                "position": [1360, 200],
                "parameters": {"respondWith": "json", "responseBody": "={{ { status: 'accepted', decisionType: $json.decisionType, decisionId: $json.decisionId } }}", "options": {}},
            },
        ],
        "connections": {
...
payloads = [
    event_hub_v4(),
    hitl_v4("MBH - 41 HITL Application Decision (V4 Final)", "mbh/v4/hitl/application/decision", "mbh-v4-hitl-application-decision", "application"),
    hitl_v4("MBH - 42 HITL Upgrade Decision (V4 Final)", "mbh/v4/hitl/upgrade/decision", "mbh-v4-hitl-upgrade-decision", "upgrade"),
]
...
out_path = Path("scripts/n8n/workflows.v4-final.created.json")
